...
class ToastErro(var view: View, var contex: Context) : ToastPersonalizado {
    override fun toastAlterarBackground(imageView: ImageView) {
        view.background = contex.getDrawable(R.drawable.shape_toast_erro)
        toastAlterarImagem(imageView)
    }

    private fun toastAlterarImagem(imageView: ImageView) {
        imageView.setImageDrawable(contex.getDrawable(R.drawable.icone_erro))
    }
